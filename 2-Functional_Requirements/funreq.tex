...
\noindent\fbox{
    \parbox{\textwidth}{
{\bf ANSWER:}\\
\textcolor{purple}{(Massi) It is actually unique also "per channel", so "For each transfer operation ∈ {credit, debit} and each pair of accounts acc, acc0 \textbf{and each channel} there is at most one tt ∈ TransferType," (in bold my addition). The reason is because a user can perform a transfer from different channels, so for each channel I need a valid TT.
}

\textcolor{green}{(Luca) That's true for now because we want to simplify the usability by the users and give they only one valid TT, not a selection of TTs
}}}
...
\noindent\fbox{
    \parbox{\textwidth}{
{\bf ANSWER:}\\
\textcolor{purple}{(Massi) Yes, customfield info parameters should be a parameter of transfer, because we need to check them, and also because when the transfer is written in the ledger, also the custom fields are written.
}
...
\textcolor{green}{(Luca)  I'm also confused about the question, input parameters are already discussed, there is only an added item, CustomField. But I'm not sure about the question is intended for.}
}}
...
\noindent\fbox{
    \parbox{\textwidth}{
{\bf ANSWER:}\\
\textcolor{purple}{(Massi) We are asking to Anna Laura a list of errors that make sense on the user perspective. I hope we have that list soon.}
}}
...
\noindent\fbox{
    ...
    \textcolor{red}{
{\bf Q5:}\\
to Luca/Massi:  Is this $Match$ definition for $debit$ correct or should the account parameters be inversed so that a $debit$ $Match$ holds if the $credit$ $Match$ holds on the accounts with inverted order?
}}}
\noindent\fbox{
    \parbox{\textwidth}{
{\bf ANSWER:}\\
\textcolor{purple}{(Massi) Since operation is a parameter of transfer, I think Match can implement the correct logic without the need to invert the parameters order.}
...
\noindent\fbox{
    \parbox{\textwidth}{
{\bf ANSWER:}\\
\textcolor{purple}{(Massi) Here there is a point. I think that the Sardex business logic forces that "debit transfer" can be performed only with "SardexAccount" (not domu or fee), but "credit transfer" can happen with other account types (in particular domu). So, in credit transfer we need to indicate the account (if there is more then one), but in debit we can indicate the member, because the member can have at most 1 account of type "conto sardex”.
}

\textcolor{brown}{(Paolo) @Massi: OK, looks like this should be documented somewhere...
}

\textcolor{green}{(Luca) Here we are discussing about the ever green question, account are property of users(members), are more simple to understand for users, but transactions are from an account to another account. We need to explain well. }
}}
...
\noindent\fbox{
    ...
    \textcolor{red}{
{\bf Q8:}\\
Again the issue arises which kinds of parameters should we consider in the model?
}}}
\noindent\fbox{
    \parbox{\textwidth}{
{\bf ANSWER:}\\
\textcolor{purple}{(Massi) I'm not sure which paramemeters you are talking about, Egon.
}
}}
